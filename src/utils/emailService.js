...
class EmailService {
        ...
        try {
            ...
            if (isProduction || isRender) {
                try {
                    ...
                    // Explicitly set connection family to 4
                    transporterConfig.family = 4;
                    
                    // Add localAddress to force IPv4
                    transporterConfig.localAddress = '0.0.0.0';
            ...
            console.log('📧 Configuring email with:', {
                ...
                hasPassword: !!process.env.EMAIL_PASS,
                mode: isProduction ? 'production (IPv4 forced)' : 'development (auto)',
                family: transporterConfig.family || 'auto'
            });
            ...
            // Don't verify in production - just log that we're ready
            if (isProduction || isRender) {
                console.log('✅ Email transporter configured (skipping verification in production)');
                console.log('💡 Emails will be sent when contact form is submitted');
            } else {
    ...
    async verifyConnection() {
        ...
        try {
            ...
            if (error.message.includes('ENETUNREACH')) {
                console.log('💡 Tip: Network unreachable - this is normal on some hosts');
                console.log('✅ Emails will still work for contact form (they will be sent when needed)');
            }
    ...
    async sendContactConfirmation(to, name) {
        ...
        try {
            ...
            if (error.code === 'EAUTH') {
                ...
                console.error('   → Connection failed. Check network/ISP settings.');
            } else if (error.code === 'ETIMEDOUT') {
                console.error('   → Connection timeout. Gmail might be slow.');
            }
    ...
    async sendAdminNotification(messageData) {
        ...
        try {
            console.log(`📧 Attempting to send admin notification for message from: ${messageData.name}`);
            ...
            const mailOptions = {
                ...
                html: `
                    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                        ...
                        <p style="background: #f5f5f5; padding: 10px; border-radius: 5px;">${messageData.message}</p>
